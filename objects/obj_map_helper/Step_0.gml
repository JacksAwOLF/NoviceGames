...
for (var i=0; i<10; i++) {
	if (keyboard_check_pressed( ord(string(i+1)) )){
		...
		if (selected_dropdown == -1 && i+1 <= instance_number(obj_sprite_dropdown)){
			...
			// if we have hidden the dropdown with edit mode, don't touch it
			if (instance_find(obj_sprite_dropdown, i).state == VisualState.active) {
				selected_dropdown = instance_find(obj_sprite_dropdown, i);
				selected_dropdown.dropdown_active = true;
				skip = (array_length(selected_dropdown.options) > 1);
			}
		} 
